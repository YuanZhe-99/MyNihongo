...
| Mock exam | shape gate | The same runner and the same `drillPassagePaneWidth` as JLPT practice, with a header row above the question holding the section names and the countdown. The clock is right-aligned in tabular figures so its width does not change as the digits do, and turns red in the last minute — before that a countdown in red is noise, and after it there is nothing to warn about. |
| JLPT practice | shape gate | The same runner as the quiz, with the panes the other way round: passage pane fixed at `drillPassagePaneWidth` (0.55 of the content width, clamped 360–640, then capped so the answers keep `quizAnswerPaneMinWidth` of 280), options in the rest, when `canSplitLayout`. **The larger half is the question here**, because it holds a text the learner re-reads while answering rather than a single word. See [`features/jlpt-practice.md`](features/jlpt-practice.md). |
