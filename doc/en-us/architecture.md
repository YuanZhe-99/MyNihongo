...
lib/
  ...
  features/
    ai/
      services/
        ai_assist_service.dart
        aicore_sentence_enhancer.dart
        genai_backend.dart
        prompt_builder.dart
        response_parser.dart
      widgets/
        ai_explanation_card.dart
        ai_settings_tiles.dart
    content/
      models/
...
assets/content/
  ...
  grammar/n5.json
  kana_notes.json
  prompts/sentence_explain.json   templates for the optional on-device model
...
- `test/ai_assist_service_test.dart` — the on-device AI policy against a fake backend; the
  load-bearing case is that a switched-off service never reaches the platform at all.
- `test/ai_prompt_builder_test.dart`, `test/ai_response_parser_test.dart` — what the model is asked,
