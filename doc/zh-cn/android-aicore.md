...
1. 按偏好顺序依次尝试各变体。
2. 保留第一个 `checkStatus()` 不是 `UNAVAILABLE` 的那个。
3. **报告哪一个变体应答了，哪些被拒绝了。**

第 3 步不是锦上添花。没有它，「本设备不支持」这一句话就同时覆盖四个不同的请求，任何现场反馈都无法把它们区分开。本应用会在状态行下面打印 `FeatureStatus=0 · refused: stable/full, stable/fast, preview/full, preview/fast`。
