...
| `forScenarioReply` | 方法 | B | 情景脚本演完之后，请模型以角色身份接话。 |
...
`forScenarioReply` 是唯一一个会随使用而变长的提示词，因此也是唯一一个会做截断的。脚本从最旧的一端整行整行地砍，自由轮次只发最后 `maxScenarioTurnsInPrompt` 个：一段对话没有天然的长度，而一句回话需要的是情景和刚刚说了什么。把一行砍成半句，等于让模型把一个残片当成话来读。它的规则禁止纠正学习者——纠正是校对的活儿，而且在问这个之前，校对已经在学习者自己那句上跑过了。
