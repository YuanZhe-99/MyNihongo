...
`VocabEntry` 是内置内容中的一个单词条目：id、JLPT 级别、词条（有汉字时为汉字，否则为读音）、读音、可选罗马音、词性标签、按语言分键的释义、日语释义的读音，以及例句。`fromJson` 在缺少 id、级别或读音时返回 null。见 [../../../../features/content-catalog.md](../../../../features/content-catalog.md)。
...
`jaReadings` 来自 JSON 键 `jaReading`，保存日语释义的平假名读音，每个义项一个，与 `meanings['ja']` 平行——`LocalizedStrings` 无法为它们携带读音。没有日语释义时它为空，这里没有读音的义项则不带注音绘制。`import_vocab.dart` 从 `assets/content/vocab_ja.json` 写入两者（见 `applyJapaneseOverlay`）；单词条目卡片与详情面板用 `FuriganaText` 绘制它们。
