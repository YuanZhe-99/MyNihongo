...
**每道题都会问两次。** 第一次调用把题写出来；第二次把它**不带**自己给出的答案交回给模型，请模型自己作答，并说这道题是否成立。只有当模型选到同一个选项、并且认为它成立时，这道题才会被保留。把答案摆在模型面前请它认可，它会认可；所以第二次刻意看不到第一次的答案：两次必须一致的独立推导才是检查，一个橡皮图章不是。沉默等于丢弃，和这里其他所有的拒绝一样。
...
| `accepts` | 静态方法 | B | 一道被判定过的题能否出现：答案一致，且被判为成立。 |
| `_survivesReview` | 方法 | B | 让模型回答它自己出的题，并作出判定。 |
