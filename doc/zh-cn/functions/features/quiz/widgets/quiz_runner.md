# lib/features/quiz/widgets/quiz_runner.dart

在屏幕上运行一次会话：题目、作答控件，以及两者之间的反馈。窗口形状合适时分成两栏，否则堆叠；门控是 `canSplitLayout`，与应用里其他每处分栏所用的相同。

题目栏可以在题面上方带一个页头，也可以为题目所关于的东西带一个前置组件，而且屏幕上的即时判分可以关掉——这正是计时的 JLPT 部分需要、而练习测验不需要的三样东西。

## 声明

| 声明 | 种类 | 层级 | 用途 |
|---|---|---|---|
| [`QuizRunner`](#runner) | 类 | A | 在屏幕上运行一次会话。 |
| `passageTextOf` | 字段 | B | 这道题所依据的文本，供答错时下面的 AI 操作使用。 |
| `_submit` | 方法 | B | 通过会话自己的 `checker` 提交已组合好的作答，键入的答案还会请模型给出第二意见；给模型看的是目录的答案文本（`answerText`），不是归一化后的键。 |
| `_advanceIfUnmarked` | 方法 | B | 在屏幕上不即时判分时直接前进。 |
| `_continue` | 方法 | B | 越过反馈进入下一题。 |
| `_syncFocus` | 方法 | B | 让快捷键节点取得键盘焦点，除非一道未作答的输入题需要它。 |
| [`_bindings`](#bindings) | 方法 | A | 构建此刻适用的键盘快捷键。 |
| `_compose` | 方法 | B | 记录正在组合的作答；点击和按键走的是同一条路径。 |
| `_placeFragment` | 方法 | B | 放入排序题剩余片段中的第 n 个。 |
| `_digitKeys`、`_numpadKeys` | 顶层常量 | B | 数字行和小键盘上的 `1` 到 `9` 键。 |
| `build` | 方法 | B | 在快捷键之内构建题目与作答控件，分栏或堆叠，并带桌面上的按键提示。 |
| [`_feedback`](#feedback) | 方法 | A | 说明作答是否正确，以及正确答案是什么。 |
| `_QuestionPane` | 类 | B | 屏幕上提问的那一半：页头、进度、前置组件、题面。 |
| `didUpdateWidget` | 方法 | B | 题目变化时重新朗读，比较的除条目与模式外还有 `questionId`。 |
| [`_instruction`](#instruction) | 方法 | A | 用文字说明学习者被要求做什么。 |
| `_modeInstruction` | 方法 | B | 为没有自带说明的题目，说明这种模式在问什么。 |
| `QuizModeLabel` | 扩展 | B | 用学习者的语言为测验模式命名。 |

## 文档

### `class QuizRunner` <a id="runner"></a>

- **种类：** 类
- **用途：** 在屏幕上运行一次会话。
- **输入：** 要运行的 `session` 与 `onFinished`；可选的题面上方 `header`、为题目所关于之物准备的 `leadingBuilder`、是否 `showFeedback`，以及布局分栏时使用的 `questionPaneWidth`。
- **返回：** 一个组件（widget）。
- **副作用：** 它自己没有；判定作答是会话的副作用。
- **算法：** 构建题目栏与作答列，然后要么把它们堆进一个 `ListView`，要么并排放置，题目栏取 `questionPaneWidth(content)`。
- **使用：** `quiz_page.dart`，以及考试页。
- **说明：** 会话由上层页面拥有，并由它销毁。这四个可选参数是为考试页而设的，其默认值就是每一个既有调用方本来得到的东西，因此练习测验没有任何变化。

  `leadingBuilder` 是一个构建器而不是一个组件，因为它随题目变化，而只有运行器知道屏幕上是哪一道题。`showFeedback` 在模拟考试中为 false，那里的卷子是最后统一判分的：每答一道就被告知结果是练习教人的方式，也恰恰是真实考试最明显不做的事，所以同一个运行器必须两者都能做。`questionPaneWidth` 默认为 `quizQuestionPaneWidth`；考试页传入 `drillPassagePaneWidth`，因为阅读题比一个词需要更多空间。

### `Widget _feedback(BuildContext context, AppLocalizations l10n, QuizOutcome outcome, QuizQuestion question)` <a id="feedback"></a>

- **种类：** 方法
- **用途：** 说明作答是否正确，以及正确答案是什么。
- **输入：** 已判定的结果，以及它所关于的那道题。
- **返回：** `Widget`。
- **副作用：** 无。
- **算法：** 用主色或错误色显示一个图标和一个词，答错时另附期望答案，有模型评语时附上评语，并就学习者所选的那个选项给出 `WhyWrong`；最后，凡是归在 `grammar:` id 下的题都跟一个 `GrammarPointChip`。
- **使用：** 位于作答控件与「继续」按钮之间，且仅在 `showFeedback` 开着时。
- **说明：** 答错之后总是显示正确答案。条目会在会话内被重新排队，而没被告知答案就重排的条目只会被再猜一次，而不是学会。`showFeedback` 关闭时，题目之间没有任何东西可读，所以 `_advanceIfUnmarked` 直接前进，而不是把学习者的时间花在一个「继续」按钮上。

  这个纸片按钮是测验第一次通向它所取材的目录。答对答错都显示：答对之后正是学习者想读一读规则的另一个时刻，而且到那时语法点本来也已经在屏幕上了。

### `Map<ShortcutActivator, VoidCallback> _bindings(QuizQuestion question, bool answered)` <a id="bindings"></a>

- **种类：** 方法
- **用途：** 构建此刻适用于屏幕上那道题的键盘快捷键。
- **输入：** 当前的 `question`，以及它是否 `answered`。
- **返回：** 一个映射，交给同时包住两种布局的 `CallbackShortcuts`。
- **副作用：** 无。
- **算法：** 模型判分期间什么都不绑定。已作答时，Enter 继续。未作答的输入题上，什么都不绑定。其余情况：数字 `1`–`9`（数字行和小键盘）选择一个选项，或放入剩余片段中的第 n 个；有已放入的片段时，Backspace 把最后一个取回；已组合出答案时，Enter 检查；题目有音频且存在日语语音时，`R` 重播音频；`S` 跳过一道生成的题目。
- **使用：** `build`，只一次，包在堆叠布局和分栏布局两者之外，因此每个承载运行器的页面都得到同样的按键。
- **说明：** **绑定只在其操作可行时才存在。** 只要激活器匹配，`CallbackShortcuts` 就会消耗该按键，所以在回调里提前返回仍然会把它吞掉。这就是未作答的输入题什么都不绑定的原因：数字必须作为文本到达输入框，Enter 必须到达输入框的 `onSubmitted`。按键事件从获得焦点的节点向根部传递，所以快捷键位于它们借以监听的那个 `Focus` 的*外面*；反过来的话它们什么也听不到。`_syncFocus` 在每次会话变化之后把焦点交给那个节点，只有未作答的输入题例外，其输入框自己取得焦点。

### `String _instruction(AppLocalizations l10n)` <a id="instruction"></a>

- **种类：** 方法
- **用途：** 说明学习者被要求做什么。
- **输入：** `l10n`。
- **返回：** `String`。
- **副作用：** 无。
- **算法：** 题目自带非空的 `instruction` 时返回它；题目带 `authored` 时返回空串，于是它上面什么都不画；否则交给 `_modeInstruction`，由它按模式分支——并把生成的题交给填空那句话，而不是 `grammarPattern` 自己的那句。
- **使用：** `_QuestionPane` 中题面上方那行灰色文字。
- **说明：** 每种模式都用文字说明，而不是依赖题目的外形，因为活用题的空格和助词题的空格长得一模一样。自带说明文字的题目保留它自己的：卷子是逐个大問写说明的，而两个在屏幕上看起来一模一样的大問要求的是不同的事情——「＿の言葉の読み方」与「＿の言葉の書き方」是同一个句子标记了同一段。把按模式分支的部分拆到 `_modeInstruction` 里，正是让这两条规则不至于纠缠在一起的做法：一条回答「这道题要求什么」，另一条回答「这种模式要求什么」，而只有后者有默认值。

  单元里自己编写的题和生成的题都归在 `QuizMode.grammarPattern` 之下，所以题型对这两者都引不对：「这句用了哪个语法点？」曾经就压在一个由模型写出来的空格上面。现在前者上面什么都不显示，后者显示填空那句话，并由 `GrammarPointLine` 在它上面写出这道题被要求考的语法点。

`passageTextOf` 是 `leadingBuilder` 隔一层的同一个想法：画出卷子的页面有那些文章，运行器没有，所以
它交出的是一个回调而不是一份映射。回调返回的是这道题所依据内容的纯日文，以及它是不是听来的，然后
直接进入 `WhyWrong`——这就是「文章哪里不是这么说的？」出现在读解题下面、「答案在哪一句？」出现在听
解题下面的原因。没有拿到它的运行器，显示的就是它一直显示的那段说明。
