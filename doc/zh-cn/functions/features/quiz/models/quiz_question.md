...
`authored` 说明这道题自己的 `prompt` 本身就是完整的题目。单元文件里的题是按人问问题的方式写的——「部屋に猫が＿。」「哪一句是礼貌的说法？」——所以在它上面再加一行题型自己的话，引出的会是另一个问题。单元里自己编写的题不管问的是什么，一律归在 `QuizMode.grammarPattern` 之下，所以题型对它并不可信，于是改由这个标志来把那一行去掉。
