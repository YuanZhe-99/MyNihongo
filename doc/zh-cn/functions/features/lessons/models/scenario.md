...
| `Scenario.partnerSpeaker` | getter | B | 学习者一直在跟谁说话：脚本在最后一个分支之后交给的那个说话人。 |
...
`partnerSpeaker` 回答的是只有自由对话才会提的一个问题：脚本演完、学习者打出新的一句之后，接话的是谁？是最后一个分支**之后**的那一行，因为那一行本来就是作为对学习者的回答写下来的；没有的话，就取脚本的最后一行。弄反了，模型就会自己回答自己。
