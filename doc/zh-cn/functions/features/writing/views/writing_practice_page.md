...
| `_rubric` | 方法 | B | 显示清单，并提供模型关于「接下来可以试什么」的建议。 |
| `_askRubric` | 方法 | B | 就清单查出的结果，问模型接下来可以试什么。 |
...
`_rubric` 位于用词那一行和逐句分析之间：先是清单，再是模型写在它下面的建议。这个顺序就是整个设计。
测量结果是应用自己的答案，在每一台设备上都会运行；建议是对它们的评述，而没有模型的手机显示清单、
不显示按钮——这也是本应用里其他每一个 AI 操作早已遵守的规则。

清单的最后一行用文字说明其中没有任何分数。JLPT 没有作文，所以这里出现一个数字，等于应用凭空造出一
场没有人考的考试。`_askRubric` 把清单自己查出的结果放进提示词，所以模型不会被问"这段写得好不好"；
它看到的是测出来的东西，被问的是接下来该怎么办，而这项任务的规则禁止重新评分。见
