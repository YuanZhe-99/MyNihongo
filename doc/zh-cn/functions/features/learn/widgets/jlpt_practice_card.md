...
学习标签页通往 JLPT 练习的入口：每个部分一行、继续一份保存的考试的提议，备考程度档位，以及模拟考试、结果、该练什么三个按钮，分别通往 `/exam`、`/exam-history` 与 `/weakness`。
...
| [`_card`](#card) | 方法 | A | 构建练习行、备考程度档位、保存的考试的提议，以及三个按钮。 |
| [`_startMock`](#startmock) | 方法 | A | 开始一场新的模拟考试，如果已经保存了一份就先询问。 |
| `_discard` | 方法 | B | 扔掉保存的考试，在一个说明什么会留下的对话框之后。 |
| [`_readiness`](#readiness) | 方法 | A | 说出学习者看起来准备到什么程度，以及是什么在拖着它。 |
| `_bandName` | 方法 | B | 用学习者的语言说出一个备考程度档位。 |
...
- **算法：** 在 `!ready` 期间把 `hasVoice` 当作 true，然后是一个 `Card`，里面放着标题、正文那一行、每个 `DrillSection` 取值一个 `_sectionRow`、一条分隔线、`_readiness`、`savedExamProvider` 有卷子时的「继续／放弃」这一对，最后是模拟考试、结果与该练什么按钮。
...
- **种类：** 方法
- **用途：** 说出学习者看起来准备到什么程度，以及是什么在拖着它。
- **输入：** `context`、`ref`、`l10n`、`theme`，以及设备是否有日语语音。
- **返回：** `Widget`。
- **副作用：** 无。
- **算法：** 监听 `readinessProvider` 与 `weaknessReportProvider`；渲染档位名称、那句说明、两行条件性的补充，以及最多三个点名最薄弱条目的标签。
- **使用：** 学习页卡片，位于练习行与模拟考试控件之间。
- **说明：** **那句说明不是可选的，也不是提示气泡。** 一个显示在 JLPT 几个字母旁边的档位，除非屏幕在同一段里说明它不是 JLPT 成绩，否则一定会被当成 JLPT 成绩来读——JEES 不公布原始分到量表分的换算，所以没有任何应用能算出真正的成绩。

  `cappedByCoverage` 和缺少日语语音各自会加上自己的一行，因为一个档位如果不解释它为什么不再往上走，读起来像 bug 而不像说明。

  最薄弱的三个点在这里点名，而不是只放在它们自己的页面上，因为一份薄弱点报告的全部价值就在于它不用去找就能被看见。
