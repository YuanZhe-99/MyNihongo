...
`_WeaknessNote` 位于第三张表下面，它是一个独立的控件，好让页面本身保持无状态：几张表只是报告的函
数，而只有这一张卡片有一个正在进行的请求需要记住。开关关着时它什么都不渲染——既不是一个禁用的按
钮，也不是一句"去打开某个东西"的邀请。没有它，报告也是完整的。

进入那条提示词的只有应用已经算好的东西，而任务的规则禁止推测学习者能不能通过考试。备考程度档位是
在明说的规则下推导出来的，旁边再放一个猜出来的档位，等于对同一个问题给出第二个无法解释的答案。大
问按它们的日语名称写进去，词按词条本身而不是它的目录 id 写进去，因为 `vocab:jm1578850` 对模型没有
任何意义，而这段建议的意义就在于它说的是关于日语的话。
