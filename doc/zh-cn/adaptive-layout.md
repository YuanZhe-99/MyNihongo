...
| 句子实验室、写作练习 | 形状门槛 | `canSplitLayout` 通过时，输入栏固定为 `labInputPaneWidth`（内容宽度的 0.40，夹在 320–460，再上限化以保证结果栏至少有 `labResultPaneMinWidth` 的 360），分析占其余部分；否则上下堆叠。**四个分析部分在任何尺寸下都留在结果栏内的单列中**——它们是一条链，结构引用词，语法引用结构，问题引用两者，把引用放在被引用旁边会让阅读顺序变得含混。分栏新增的是*输入*的那一栏：题目、输入框、按钮和历史记录，分析并不引用它们中的任何一个。低于门槛时，历史记录移到顶栏按钮后打开的 sheet 里，因为在输入框和答案之间插一份列表会把答案挤出屏幕。 |

**明知而接受的代价：** 横持的手机（915 × 412）保留每个单列布局，尽管它是所有视口中高度最少的——本来受益最大的情形。本系列为其分栏表面的一致性接受了这个取舍；本应用沿用。

**关于句子实验室的规则，它在 `v0.3.2` 变了。** 它此前在任何尺寸下都是单列，并作为刻意例外记录在这里，理由是它的各部分构成一条链。这个例外划得太宽了：不可分开的是那条*链*，而输入框和历史记录并不属于它。把它们分到自己那一栏，链本身保持原样——单列、按顺序——同时让展开的手机不必把 933 dp 用在留白上。例外所保护的规则依然在被执行；只是执行在了正确的对象上。
