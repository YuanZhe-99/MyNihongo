...
有三种记录与其他记录共享这个文件，但并不指向任何目录条目：

- **`profile:me`**——学习者的目标级别、每日上限与学习连续天数。
- **`lesson:<id>`**——一节课的结果，来自第三阶段 M3.3。它的计数器表示通过与未通过而不是作答次数，并且从不参与调度。
- **`lab:<hash>`** 与 **`writing:<hash>`**——句子实验室与写作练习的历史记录，来自 `v0.3.2`。它们的计数器恒为零：没有人作答过任何东西。

`ProgressData.studyRecords` 是其中*确实*被学习的子集，因此「已记录条目数」与复习队列都会跳过上述三种。
...
一条被记住的句子是一条记录，它的载荷放在 `extraJson` 的 `history` 键下，和学习者档案用的是同一套做法，理由也相同：
...
**只保存输入。** 分析是文本与随应用发布的目录的纯函数，所以在打开时重新算出来，而不是把产生它的那个版本的结果冻结下来；模型生成的任何内容都不写在这里，这是 `AGENTS.md` 里的行为契约。`unitId` 只在写作练习来自某个单元时才存在。

**id 是内容的哈希**，不是随机值：`<前缀>:<16 位十六进制>`，取自单元 id 与做过空白归一化的文本。由此有三件事成立。同一个句子分析两次会更新同一条记录并把它移到最前，而不是让历史里塞满重复项；两台设备分析了同一句话会得到相同的 id，从而合并成一条而不是各自积累；而同一个句子为两个不同练习写下时仍是两份作业，因为单元也是键的一部分。哈希是两条 FNV-1a 32 位通道，以移位加法形式计算，所以结果不依赖于平台上 `int` 的宽度。

每种各最多保留 **100 条**。进度文件在每次同步时被整份下载、合并、上传，所以一份无上限的「凡是打过的字都记下来」的日志，最终会比它所搭载的进度本身还贵。裁剪按种类进行，因此繁忙的句子实验室不会把写作历史清空，而且只会移除历史记录。删除是真删除而不是墓碑：三方合并把「一侧删除、另一侧未改动」的记录视为已删除，所以忘掉一个句子就是在各处都忘掉它。

### 考试历史记录

一次 JLPT 卷子的作答是一条记录，它的载荷放在 `extraJson` 的 `exam` 键下，和学习者档案与句子历史记录用的是同一套做法，理由也相同：记录能免费获得逐记录的三方合并、冲突对话框、同步与备份，而第二个数据模块则意味着第二个远程文件、第二个备份条目，以及十一份要重新录制的黄金记录。
...
**只保存输入。** 也就是问了哪些题，以及对每道题的第一次作答是什么：`1` 为答对，`0` 为答错，`-1` 表示因为时间用完而从未作答。题目文字、它的选项和它的解析都在显示结果时从随包发布的真题练习文件里读回来，因此一次修正了答案键的内容更新会连同历史记录一起修正，而不是留下一个随包文件已经不再认同的冻结分数。随包文件里已经没有的题目会用一行说明，而不是从这次作答里消失。

`-1` 刻意是它自己的一个取值。一道未作答的题既不算对也不算错：把它算作错，会让每一个计时分数都比学习者实际的表现更难看；而把它丢掉，则会让每一个计时分数都更好看。

**id 带时间戳并加盐**，而不是由内容推导。时间戳让这些 id 按作答发生的顺序排列，从而使文件 diff 可读；四位十六进制后缀则是阻止两台设备上在同一秒开始的两次作答被合并的东西。这与句子历史记录的规则正好相反，而且是刻意的：重新分析同一个句子应该更新同一条记录，而同一份卷子的两次作答确实是两件事。

**部分的键是普通字符串**，所以 `progress/` 不从 `drills/` 导入任何东西。后续版本新增的一个部分，在旧版构建里仍能原样往返，而不是让它赔上这次作答的其余内容。

`correct` 与 `wrong` 携带整卷的合计，尽管载荷里也有它们。它们是旧版构建的同步冲突对话框所读取的内容，而一次在那里显示「正确 0 · 错误 0」的作答，会就一条它本来就无法解释的记录，向那个构建传达不实的信息。`studiedKinds` **不会**新增 `exam`，所以一次作答从不被调度、从不计入条目数，也从不进入复习队列——它问过的那些题，已经在被作答时逐个推动了各自条目的间隔。
...
| 开发者选项 | `storage_config.json` | 否 | 版本号那一行被连点八次后为 `true`；缺失表示关闭（`debugMode`）。刻意不同步——它揭示的是这一台设备的诊断信息，换到别处时其中每一个数字讲的都是另一台手机 |
...
对它的每一次写入都走同一个队列，而每一次写入都在队列内部重新读取这个文件。同一瞬间改动的两项设置，过去读到的是同一份文件、写出的是两个不同的后继版本，于是后一个抹掉了前一个的键；在 Windows 上它则直接抛出异常，因为后一次写入把自己的临时文件改名覆盖到了前一次还开着的那个文件上。一个人快速拨动两个开关就足以触发。
