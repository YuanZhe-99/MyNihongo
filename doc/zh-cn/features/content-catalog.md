...
学习者可以把应用设为日语。日语界面下读到什么，按内容种类分别决定：

| 内容 | 日语读者看到什么 |
|---|---|
| 单词的含义 | 一条**日语释义**，国語辞典式——「学校で勉強を教える人」，而不是翻译——每个词一到三条，带注音 |
| 语法点 | 一行带注音的含义，以及一段用浅显日语写的解释 |
| 功能词的色块 | 一行日语释义 |
...
**释义带读音。** 单词的释义和语法点的一行含义是学习者最常遇到的日语文本，所以两者都带平假名读音并以注音绘制：条目上的 `jaReading`（每条释义一个），以及语法点上的 `meaningJaReading`。较长的散文——解释、题目提示——与在其他每种语言下一样是纯文本。

有两样东西在任何界面语言下都保持不变。语法点的 `structure`（`N + です`、`V-て + いる`）保留 N/V/A 记法，日语的语法参考书也是这样书写结构的。词性标签如今在每种语言下都通过界面自己的字符串命名，而过去无论何种语言都印成 `verb-godan` 和 `suru-verb`。
