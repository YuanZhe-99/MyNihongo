...
- **PageUp 和 PageDown 滚动参考列表**（假名、单词、语法），无需列表内部有任何东西取得焦点。在桌面上，滚动视图只有在自己要求时才会挂到路由的主滚动控制器上，而这两个按键也只有通过那个控制器才能到达一个内部无焦点的列表，所以这三个列表都设了 `primary: true`；在 0.5.3 之前，这两个按键在列表内某个东西取得焦点之前什么都不做。方向键在桌面上不是滚动键：它们像 Flutter 里其他任何地方一样在条目之间移动焦点，取得焦点的条目会被滚动进视野。`keyboard_scroll_ui_test` 在 Windows 窗口尺寸下驱动了这一行为。
