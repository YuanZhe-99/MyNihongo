...
class PrivacyPolicyPage extends StatelessWidget {
  ...
  /// as its own string.
  String _getText(Locale locale) => switch (locale.languageCode) {
    'ja' => _ja,
    'zh' => locale.countryCode == 'TW' ? _zhTw : _zh,
    _ => _en,
  };
...
MyNihongo!!!!! をご利用いただきありがとうございます。私たちはあなたのプライバシーを大切にしています。このプライバシーポリシーでは、アプリがあなたのデータをどのように扱うかを説明します。

データの収集

MyNihongo!!!!! は、個人情報を収集、送信、共有することは一切ありません。アプリには解析ツール、広告トラッカー、クラッシュレポート、その他いかなるデータ収集の仕組みも含まれていません。アカウントも、アプリ独自のサーバーもありません。

データの保存

アプリが作るデータ（学習の進み具合と設定）は、すべてあなたの端末に保存されます。保存先はアプリ自身のフォルダ、または設定で選んだフォルダです。かな・語彙・文法のコンテンツはアプリに同梱されており、どこかへ送られることはありません。

ネットワークへのアクセス

MyNihongo!!!!! がインターネットにアクセスするのは次の場合だけで、どれもあなた自身が有効にしたときに限られます。

• WebDAV 同期：WebDAV クラウド同期を有効にすると、アプリは学習の進み具合を、あなた自身が設定した WebDAV サーバーに送ります。それ以外のサーバーにデータを送ることはありません。
• ネットワーク音声認識：下の「音声認識」で説明するスイッチを入れた場合のみです。初期状態ではオフです。
• 端末上の AI モデルのダウンロード：「端末上の AI アシスト」を有効にし、「ダウンロード」をタップした場合のみです。ダウンロードを行うのはアプリではなく、Android の AICore システムサービスです。

これ以外のネットワーク通信は行いません。アプリが INTERNET 権限を求めるのはこのためだけです。

はっきり書いておきたい点が二つあります。

• WebDAV の認証情報は、このシリーズの他のアプリと同じく、アプリのフォルダに平文で保存されます。そのフォルダにアクセスできる人（そのフォルダを含む端末のバックアップも含みます）は読むことができます。サーバーがアプリ用パスワードに対応している場合は、メインアカウントのパスワードではなくそちらを使ってください。

• 同期の実行中は端末がスリープしないようにしています。アップロードの途中で同期が中断されないためです。同期が終わるか失敗すると、すぐに解除されます。

データのバックアップ、エクスポート、インポート

アプリには、端末内へのバックアップ機能と、ZIP のエクスポート・インポート機能があります。バックアップファイルは端末に保存され、エクスポートはあなたが選んだフォルダに書き出されます。どちらにも学習の進み具合が含まれます。その後これらのファイルをどこへ置くかは完全にあなた次第で、アプリがアップロードすることはありません。

フォルダやファイルを選ぶときはシステムの選択画面を使います。これはアプリにその場所一つへのアクセスだけを与えるもので、ストレージの権限は必要ありません。

音声合成

アプリは、かな・単語・例文などの日本語を読み上げることができます。これには端末にすでに入っている音声合成エンジン（Android、Apple、または Windows の音声機能）を使います。テキストはそのエンジンに渡され、音声は端末の中で作られます。どこにも送られず、何も録音されず、音声ファイルも作られません。選んだ話す速さと声は端末にだけ保存されます。

日本語の音声が入っていない場合は何も読み上げず、アプリがそのことを知らせます。音声のインストールはアプリではなく、システムの設定で行います。

音声認識

発音練習では、あなたの声を聞き、音声認識が聞き取った内容を項目の読みと比べます。認識は端末にすでにある音声サービス（Android、Apple、または Windows の音声機能）が行います。

初期状態では、アプリはオフラインでの認識だけを求めます。オフラインの日本語データが入っていない端末では認識が失敗し、声をこっそりどこかへ送る代わりに、アプリがそのことを伝えます。設定には、初期状態でオフになっているスイッチがあり、システムのネットワーク認識への切り替えを許可できます。これをオンにした場合に限り、話した内容が文字起こしのためにシステムの音声サービスへ送られることがあります。多くの Android 端末では、そのサービスは Google のものです。

音声がファイルに録音されることはなく、話した内容が保存されることもありません。マイクは練習を録音している間だけ使われ、権限は初めて録音ボタンをタップしたときに理由とともに求められます。

端末上の AI アシスト

Google の AICore システムサービスがある Android のスマートフォンでは、センテンスラボで、アプリ自身の指摘をさらに詳しく説明したり、文を説明したり、書き換えを提案したりできます。これは初期状態でオフで、設定で「端末上の AI アシスト」をオンにするまで何も起こりません。

オンにすると、入力した文が、アプリ自身による分析とアプリ自身の文法ノートとともに、AICore を通じてスマートフォンの中で動くモデルに渡されます。サーバーには何も送られず、何も記録されず、生成された内容も保存されません。文を変えたりページを離れたりすると消えます。

この機能の中でネットワークを使うのは一か所だけで、それもあなたが求めたときだけです。ある機能で初めて「ダウンロード」をタップすると、AICore システムサービスが Google からそのモデルを取得します。ダウンロードを行うのはアプリではなく、アプリがそれと一緒に何かを送ることもありません。モデルがスマートフォンに入ったあとは、ネットワークなしで動きます。

生成された文には、生成されたものであることが必ず表示されます。間違っていることもあり、アプリ自身の分析を変えることはなく、アプリの辞書や文法コンテンツに書き込まれることもありません。

毎日のリマインダー

選んだ時刻に、一日一回、学習するようアプリに知らせてもらうことができます。オンにするまでは何も起こらず、通知の権限はオンにしたときに初めて求められます。設定を開いただけでは何も求めません。

リマインダーの内容は、あなた自身の進み具合のファイルから端末の中で作られ、端末が表示します。その内容（期限の来た項目がいくつあるか、次の単元は何か）が、表示されたスマートフォンやパソコンの外に出ることはありません。リマインダーがサーバーに送られることはなく、それを開いたかどうかも記録されません。

スマートフォンではオペレーティングシステムが予定を管理するため、アプリが動いていなくてもリマインダーは届きます。パソコンではアプリ自身が通知を出すため、アプリが開いている間しか届きません。

このポリシーの変更

このプライバシーポリシーは随時更新されることがあります。更新版は、アプリ内または関連する配布チャネルで公開されます。音声合成、音声認識、端末上の AI アシスト、毎日のリマインダーについては上で説明したとおりです。今後、端末の外へ何かを送る可能性のある機能を加える場合は、公開前にここで説明し、あなたがオンにしない限りオフのままにします。''';
}
